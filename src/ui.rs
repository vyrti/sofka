        ...
        .map(|(row_key, obj)| {
            ...
            for (i, cell) in base_cells.iter().enumerate() {
                ...
                let volatile = col_visible(ns_off + i)
                    .then(|| spec.volatile(obj, &app.kind_plural, i, now))
                    .flatten();
                match volatile {
                    Some(value) => cells.push(TableCellText::Owned(value)),
                    None => cells.push(TableCellText::Borrowed(cell.as_str())),
                }
            }
...
fn render_name_cell<'a>(app: &App, name: &'a str, base: Color) -> Cell<'a> {
    ...
    let hl = Style::default()
        .fg(theme::yellow())
        .add_modifier(Modifier::BOLD);
    // The matcher reports positions ascending and deduplicated, so walking
    // them alongside the char boundaries makes each run a slice of `name` —
    // neither the set nor the per-run strings the old version built are
    // needed. The list is shared out of the per-frame memo, so it is read,
    // never reordered.
