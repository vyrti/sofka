...
| `:oha` / `:oha <dur> [conns]`                 | benchmark the selected ingress/service/pod with oha; tries the advertised address, then a port-forward; shown only when `oha` is executable on `PATH`         |
