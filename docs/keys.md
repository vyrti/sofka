...
| `:oha` / `:oha <dur> [conns] [port=N]`        | benchmark the selected ingress/service/pod with oha; ranks the declared ports to find the HTTP one unless `port=` names it; tries the advertised address, then a port-forward; shown only when `oha` is executable on `PATH` |
