...
| `:oha` / `:oha <dur> [conns] [port=N]`        | benchmark the selected ingress/service/pod with oha; prefers the port that declares HTTP unless `port=` names one; tries the advertised address, then a port-forward; shown only when `oha` is executable on `PATH` |
